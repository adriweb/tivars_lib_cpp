...
namespace tivars
{
    ...
    static string makeStringFromReal(data_t data, const options_t& options)
    {
        data_t::reference type = data[0];
        if (type != 0x1B && type != 0x9B && type != 0x1C && type != 0x8C)
        {
        ...
        return frac ? dec2frac(stod(str)) : to_string(stoi(str));
    }

    static string makeStringFromImag(data_t data, const options_t& options)
    {
        data_t::reference type = data[0];
        if ((type & ~0x80) != 0x1E)
        {
    ...
    {
        return makeStringFromComplex(data, options, makeStringFromReal, makeStringFromImag);
    }

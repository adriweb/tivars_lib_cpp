...
namespace tivars
{
    ...
    data_t TH_0x1E::makeDataFromString(const string& str, const options_t& options)
    {
        ...
        {
            throw runtime_error("Invalid input string. Needs to be a valid Exact Complex Pi");
        }
    }
    ...
    string TH_0x1E::makeStringFromData(const data_t& data, const options_t& options)
    {
        ...
        if (data.size() != TH_0x1E::dataByteCount)
        {
            throw invalid_argument("Empty data array. Needs to contain " + to_string(TH_0x1E::dataByteCount) + " bytes");
        }

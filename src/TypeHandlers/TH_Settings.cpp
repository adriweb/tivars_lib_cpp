...
namespace tivars::TypeHandlers
{
    namespace
    {
        ...
        const settings_field_t recall_window_fields[] = {
    ...
    data_t TH_Settings::makeDataFromString(const std::string& str, const options_t& options, const TIVarFile* _ctx)
    {
        ...
        if (type == 0x10)
        {
            ...
            {
                throw std::invalid_argument("RecallWindow JSON must be an object");
            }

            data_t data = {0xCF, 0x00};
            for (const auto& field : recall_window_fields)
            {
    ...
    std::string TH_Settings::makeStringFromData(const data_t& data, const options_t& options, const TIVarFile* _ctx)
    {
        ...
        if (data.size() == 209)
        {
            if (data[0] != 0xCF || data[1] != 0x00)
            {
                throw std::invalid_argument("Invalid RecallWindow data header");
            }

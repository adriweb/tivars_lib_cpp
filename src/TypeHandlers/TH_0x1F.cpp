...
namespace tivars
{
    ...
    data_t TH_0x1F::makeDataFromString(const string& str, const options_t& options)
    {
        ...
        {
            throw runtime_error("Invalid input string. Needs to be a valid Exact Complex Pi Fraction");
        }
    }
    ...
    string TH_0x1F::makeStringFromData(const data_t& data, const options_t& options)
    {
        ...
        if (data.size() != TH_0x1F::dataByteCount)
        {
            throw invalid_argument("Empty data array. Needs to contain " + to_string(TH_0x1F::dataByteCount) + " bytes");
        }

...
namespace tivars::TypeHandlers
{
    namespace
    {
        ...
        std::string grouped_entry_name_to_string(const TIVarType& type, const data_t& rawName)
        {
            ...
            const uint8_t typeId = static_cast<uint8_t>(type.getId());
            const uint8_t first = rawName[0];
            const uint8_t second = rawName.size() > 1 ? rawName[1] : 0x00;
            if ((typeId == 0x01 || typeId == 0x0D) && first == 0x5D)
            {
                if (second <= 0x05)
                {
                    return std::string("L") + char((second == 9 ? '0' : static_cast<char>('1' + second)));
                }
                if (second == 0x40)
                {
                    return "IDList";
            ...
            {
                return std::string("[") + char('A' + second) + "]";
            }
            if ((typeId == 0x03 || typeId == 0x0B) && first == 0x5E)
            {
                ...
                if (second >= 0x20 && second <= 0x2B)
                {
                    ...
                    return std::string(1, axis) + char('1' + idx) + "T";
                }
                if (second >= 0x40 && second <= 0x45)
                {
                    return std::string("R") + char('1' + (second - 0x40));
                }
                if (second >= 0x80 && second <= 0x82)
                {
                    return std::string(1, char('U' + (second - 0x80)));
                }
            }
            ...
            const auto nulPos = std::find(rawName.begin(), rawName.end(), 0x00);
            for (auto it = rawName.begin(); it != nulPos; ++it)
            {
                if (*it < 0x20 || *it > 0x7E)
                {
                    return "";
            ...
            return std::string(rawName.begin(), nulPos);
        }
